    ...
    reviews: [
      {
        ...
        rating: 4,
        date: "3 days ago",
        comment: "Notion AI makes content creation so much easier. It's perfect for brainstorming and organizing thoughts. The integration with Notion's existing features is seamless.",
        helpful: 19,
        verified: true
      },
      {
        ...
        rating: 5,
        date: "1 week ago",
        comment: "I use Notion AI daily for work. It helps me write better emails, create meeting notes, and organize projects. The AI suggestions are always relevant and helpful.",
        helpful: 34,
        verified: true
      },
      {
        ...
        rating: 4,
        date: "5 days ago",
        comment: "Great addition to Notion. The AI features are well-integrated and don't feel forced. Perfect for teams that already use Notion for collaboration.",
        helpful: 21,
        verified: false
      }
    ]
    ...
    reviews: [
      {
        ...
        rating: 4,
        date: "2 days ago",
        comment: "Jasper is excellent for creating marketing copy and content. The templates are great and the AI generates high-quality content quickly. Perfect for busy marketers.",
        helpful: 27,
        verified: true
      },
      {
        ...
        rating: 3,
        date: "1 week ago",
        comment: "Good tool but can be expensive for small businesses. The content quality is high but sometimes needs significant editing to match our brand voice.",
        helpful: 14,
        verified: true
      },
      {
        ...
        rating: 5,
        date: "4 days ago",
        comment: "Jasper has saved me hours of writing time. The brand voice feature works well and the content is always engaging. Highly recommend for content creators.",
        helpful: 38,
        verified: false
      }
    ]
...
async function seedRealAIData() {
  console.log('Starting to seed real AI tools data...');
  ...
  try {
    ...
    for (const tool of realAITools) {
      ...
      if (error) {
        ...
        console.log(`✅ Inserted/updated tool: ${tool.name}`);
      }
    }

    // Insert reviews
    for (const toolReview of sampleReviews) {
      // Get the tool ID
      const { data: toolData, error: toolError } = await supabase
        .from('tools')
        .select('id')
        .eq('name', toolReview.tool_name)
        .single();

      if (toolError || !toolData) {
        console.error(`Could not find tool: ${toolReview.tool_name}`);
        continue;
      }

      for (const review of toolReview.reviews) {
        const { error } = await supabase
          .from('reviews')
          .insert({
            tool_id: toolData.id,
        ...
        if (error) {
          console.error(`Error inserting review for ${toolReview.tool_name}:`, error);
        } else {
          console.log(`✅ Inserted review for: ${toolReview.tool_name}`);
        }
      }
    }

    console.log('🎉 Successfully seeded real AI tools and reviews data!');
    
    // Update the tools.json file with the new data
    ...
    console.log('📝 Updated data/tools.json with new data');
