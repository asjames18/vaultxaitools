...
// Sample reviews data (simplified)
const sampleReviews = [
  {
    user_name: 'Sarah M.',
    rating: 5,
...
// Function to add simple reviews
async function addSimpleReviews() {
  console.log('📝 Adding simple reviews to AI tools...');
...
CREATE TABLE IF NOT EXISTS public.reviews (
    ...
    console.log(`\n📈 Simple reviews addition completed:`);
    console.log(`   ✅ Total reviews added: ${totalAdded}`);
    console.log(`   📊 Average reviews per tool: ${(totalAdded / tools.length).toFixed(1)}`);
    
    // Verify the results
    await verifySimpleReviews();
    
  } catch (error) {
    console.error('❌ Error adding simple reviews:', error);
  }
}

// Function to verify simple reviews
async function verifySimpleReviews() {
  console.log('\n🔍 Verifying simple reviews...');
  ...
  try {
    ...
    console.error('❌ Error verifying simple reviews:', error);
  }
}
...
async function main() {
  ...
  try {
    // Try to add simple reviews
    await addSimpleReviews();
    
    console.log('\n🎉 Simple reviews process completed!');
