      ...
      <div className="max-w-7xl mx-auto px-4 py-16 space-y-20">
        ...
        <div className="text-center space-y-6">
          ...
          <p className="text-gray-400 text-xl max-w-2xl mx-auto">
            Agents, MCP servers, skills, and blueprints — everything you need to deploy and monetize AI automation systems.
          </p>

          {/* Trending tags */}
          <div className="flex flex-wrap justify-center gap-2 pt-2">
            {trendingTags.map(tag => (
              <span key={tag} className="px-3 py-1 text-xs text-gray-500 bg-white/5 border border-white/10 rounded-full hover:border-green-500/30 hover:text-green-400 transition-colors cursor-default">
                #{tag}
              </span>
            ))}
          </div>
          ...
          <div className="flex flex-wrap justify-center gap-3 pt-2">
            {[
              ...
              <Link key={item.href} href={item.href}
                className={`px-5 py-2.5 font-bold rounded-lg text-sm transition-colors focus-visible:outline-2 focus-visible:outline-green-500 ${item.color}`}>
                {item.label}
              </Link>
            ))}
          </div>
        ...
        {agents.length > 0 && (
          <section>
            <div className="flex items-center justify-between mb-6">
              ...
              <Link href="/agents" className="text-sm text-green-400 hover:text-green-300 transition-colors focus-visible:outline-2 focus-visible:outline-green-500">
                View all →
              </Link>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {agents.map(agent => (
                <Link key={agent.id} href={`/tool/${agent.id}`}
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    {agent.rating ? (
                      <span className="flex items-center gap-1 text-sm text-yellow-400">★ {agent.rating.toFixed(1)}</span>
                    ) : <span />}
                    <span className="text-xs text-green-400 group-hover:translate-x-1 transition-transform">View & Deploy →</span>
                  </div>
                </Link>
              ))}
            </div>
          ...
          {products.length > 0 && (
            <section>
              <div className="flex items-center justify-between mb-6">
                ...
                <Link href="/products" className="text-sm text-green-400 hover:text-green-300 transition-colors">View all →</Link>
              </div>
              <div className="space-y-4">
                {products.map(p => (
                  <Link key={p.id} href={`/products/${p.slug}`}
                    className="group flex items-center gap-4 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-green-500/30 rounded-xl p-4 transition-all duration-200 focus-visible:outline-2 focus-visible:outline-green-500">
          ...
          {servers.length > 0 && (
            <section>
              <div className="flex items-center justify-between mb-6">
                ...
                <Link href="/mcp-servers" className="text-sm text-green-400 hover:text-green-300 transition-colors">View all →</Link>
              </div>
              <div className="space-y-4">
                {servers.map(server => (
                  <Link key={server.id} href={`/tool/${server.id}`}
                    className="group flex items-start gap-4 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-green-500/30 rounded-xl p-4 transition-all duration-200 focus-visible:outline-2 focus-visible:outline-green-500">
                    <div className="w-9 h-9 rounded-lg bg-purple-500/20 border border-purple-500/30 flex items-center justify-center text-purple-400 font-bold text-sm flex-shrink-0">
                      MCP
                    </div>
                    <div className="flex-1 min-w-0">
        ...
        {skills.length > 0 && (
          <section>
            <div className="flex items-center justify-between mb-6">
              ...
              <Link href="/skills" className="text-sm text-green-400 hover:text-green-300 transition-colors">View all →</Link>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {skills.map(skill => (
                <Link key={skill.id} href={`/tool/${skill.id}`}
                  className="group bg-white/5 hover:bg-white/10 border border-white/10 hover:border-green-500/30 rounded-xl p-5 transition-all duration-200 focus-visible:outline-2 focus-visible:outline-green-500">
        ...
        <section className="border-t border-white/10 pt-16 grid md:grid-cols-2 gap-8">
          <div className="bg-white/5 border border-white/10 rounded-2xl p-8 space-y-4">
            ...
            <Link href="/creators"
              className="inline-block px-6 py-3 bg-green-500 hover:bg-green-400 text-black font-bold rounded-lg transition-colors focus-visible:outline-2 focus-visible:outline-white">
              Become a Creator
            </Link>
          </div>
          <div className="bg-white/5 border border-white/10 rounded-2xl p-8 space-y-4">
            ...
            <p className="text-gray-400">New agents, MCP server drops, and automation blueprints — every week in your inbox.</p>
