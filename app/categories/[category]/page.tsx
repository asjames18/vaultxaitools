...
export default async function CategoryPage({ params }: CategoryPageProps) {
  ...
  try {
    ...
    const allTools = (toolsData || []).map(dbTool => ({
      id: dbTool.id || '',
      name: dbTool.name || '',
      logo: dbTool.logo || '🔧',
    ...
    const breadcrumbStructuredData = {
      ...
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 2, name: 'Categories', item: 'https://vaultxaitools.com/categories' },
        { '@type': 'ListItem', position: 3, name: currentCategory.name, item: `https://vaultxaitools.com/categories/${currentCategory.name.toLowerCase()}` },
      ],
    } as const;
